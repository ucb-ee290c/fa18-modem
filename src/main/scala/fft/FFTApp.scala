...
object FFTApp extends App {
   ...
   * Parse arguments
   *
   * Some arguments are used by the fft generator and are used to construct a FixedFFTParams object.
   * The rest get returned as a List[String] to pass to the Chisel driver
   *
   */
  def argParse(args: List[String], params: FixedFFTParams): (List[String], FixedFFTParams) = {
    args match {
      case "--help" :: tail =>
        println(usage)
  ...
  val defaultParams = FixedFFTParams(
    dataWidth = 8,
    twiddleWidth = 8,
    maxVal = 2
    // xyWidth = 12,
  ...
  // Run the Chisel driver to generate a fft
  Driver.execute(chiselArgs.toArray, () => new FFT(params))
}
